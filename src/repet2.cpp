...
void init_search_context(Search_Context *context, const char *data, int length)
{
	HACKDATA = data;
...
bool search_done(Search_Context *context)
{
	...
	for (; pos < length; pos++) {
		...
		if (pos + SEARCH_TUPLE_SIZE <= length) {
			...
			do {
				...
				// TODO: Allow going past the length for repeating
...
{
	{
		FILE *file = fopen("repet2.pdb", "rb");
		static char buffer[1024*1024*2];
		int length = (int)fread(buffer, 1, 1024*1024*2, file);
		fclose(file);

		Search_Context context;
		init_search_context(&context, buffer, length);

		int match_count = 0;
		while (!search_done(&context)) {
			Search_Match matches[128];
			match_count += search_next_matches(&context, matches, 128);
		}
	...
	char input[2048] = "Longest Longer Long Longest";
	int test_len;
	do {
		char *test_data = input;
		test_len = (int)strlen(test_data);

		Search_Context context;
		init_search_context(&context, test_data, test_len);

		while (!search_done(&context)) {
			Search_Match matches[128];
			int match_count = search_next_matches(&context, matches, 128);

			for (int match_index = 0; match_index < match_count; match_index++) {
				Search_Match *match = &matches[match_index];

				puts(test_data);

				static const char chars[] = " ^~*";
				for (int col = 0; col < test_len; col++) {
					int rel_left = col - (match->position - match->offset);
					int rel_right = col - match->position;

					int char_index = ((rel_left >= 0 && rel_left < match->length) ? 1 : 0)
						| ( (rel_right >= 0 && rel_right < match->length) ? 2 : 0);
